...
// Компонент списка записей для главного экрана с переключением "Сегодня / Завтра / Неделя"
export default function AppointmentList() {
  ...
  // Группировка записей по дням для вкладки "Неделя"
  const groupAppointmentsByDay = (appts: AppointmentWithClient[]) => {
    const groups: { [key: string]: AppointmentWithClient[] } = {};
    appts.forEach((app) => {
      const dateKey = format(new Date(app.dateTime), 'yyyy-MM-dd');
      if (!groups[dateKey]) {
        groups[dateKey] = [];
      }
    ...
    return Object.keys(groups)
      ...
      .map((dateKey) => {
        ...
        return {
          date: dateObj,
          dateStr: format(dateObj, 'EEEE, d MMMM', { locale: ru }),
          appointments: groups[dateKey],
        };
      });
  };

  const renderAppointmentCard = (app: AppointmentWithClient) => {
    return (
      <div
        key={app.id}
  ...
  return (
    <div className="space-y-4">
      {/* Переключатель вкладок Сегодня / Завтра / Неделя */}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center bg-tg-secondary-bg rounded-2xl border border-slate-100 dark:border-slate-800 p-6">
          ...
          <p className="text-sm text-tg-hint max-w-xs">
            {activeTab === 'today'
              ? 'На сегодня записей не найдено.'
              : activeTab === 'tomorrow'
              ? 'На завтра записей не найдено.'
              : 'На эту неделю записей не найдено.'}{' '}
            Нажми кнопку ниже, чтобы добавить запись.
          </p>
        </div>
      ) : activeTab === 'week' ? (
        <div className="space-y-6">
          {groupAppointmentsByDay(appointments).map((group) => (
            <div key={group.dateStr} className="space-y-2.5">
              <div className="sticky top-0 bg-tg-bg/95 backdrop-blur-xs py-1.5 z-10 border-b border-slate-200/50 dark:border-slate-800/60 mb-2">
                <span className="text-xs font-bold text-tg-button uppercase tracking-wider capitalize">
                  {group.dateStr}
                </span>
              </div>
              <div className="space-y-3">
                {group.appointments.map((app) => renderAppointmentCard(app))}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {appointments.map((app) => renderAppointmentCard(app))}
        </div>
      )}
    </div>
  );
